...
def _seed_all_entities(db: Session, keyword: str = "Quantum"):
...
def test_search_across_all_entity_types(client: TestClient, db_session: Session):
    """Verify that search returns results across all supported entity categories."""
    _seed_all_entities(db_session, keyword="Quantum")

    response = client.get("/api/v1/search/search_entities?q=Quantum")
...
def test_search_type_filtering(client: TestClient, db_session: Session):
    """Verify filtering search queries by specific entity types."""
    _seed_all_entities(db_session, keyword="Filtered")

    # Filter to only tasks and notes
    response = client.get("/api/v1/search/search_entities?q=Filtered&types=tasks,notes")
    assert response.status_code == 200
...
def test_search_relevance_ranking(client: TestClient, db_session: Session):
    ...
    response = client.get("/api/v1/search/search_entities?q=Supernova")
    assert response.status_code == 200
    data = response.json()["data"]
...
def test_search_soft_deleted_excluded(
    ...
    response = client.get("/api/v1/search/search_entities?q=Target")
    assert response.status_code == 200
    data = response.json()["data"]
...
def test_search_case_insensitivity_and_snippet(
    ...
    response = client.get("/api/v1/search/search_entities?q=hyperdrive")
    assert response.status_code == 200
    data = response.json()["data"]
...
def test_search_pagination_limit_offset(
    ...
    response = client.get("/api/v1/search/search_entities?q=BatchItem&limit=3&offset=0")
    assert response.status_code == 200
    res = response.json()
    ...
    # Next page
    response2 = client.get("/api/v1/search/search_entities?q=BatchItem&limit=3&offset=3")
