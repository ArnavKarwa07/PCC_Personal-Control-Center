...
def list_contacts(
    ...
    contacts, total, total_pages = contact_service.list_contacts(
        db=db, search=search, overdue_only=overdue_only, page=page, per_page=per_page
    )
    return {
...
def create_contact(
    ...
    contact = contact_service.create_contact(db=db, data=data)
...
def get_contact_by_id(
    ...
    contact = contact_service.get_contact(db=db, contact_id=contact_id)
    if not contact:
...
def update_contact(
    ...
    contact = contact_service.update_contact(db=db, contact_id=contact_id, data=data)
    if not contact:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Contact not found")
    return {"data": ContactRead.model_validate(contact).model_dump()}
